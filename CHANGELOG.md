...
长任务一天几百张，盘越堆越多，没人会手动去清。

**改法**：
- `trimShots()`：按 mtime 新→旧排序，**数量超额**（第 201 张起）或 **时间超期**（>7 天）任一命中即回收；
- **只移不删**：收走的文件移进 `$DSH_HOME/dsh-cache-control/recycle/`（与「⑤存储」账本同区，随时捞回），
  目标重名时改名落进去，绝不覆盖回收目录里的旧件；
- **不起定时器**：截图唯一入口就是 `browser_screenshot`，写完顺手检查一次天然覆盖所有增长路径；
  定时器是多活的生命周期，收益为零。回收失败（跨卷/占用）安静跳过，绝不影响截图本身；
- 返回值在真的收了东西时带 `recycled: N`，没收就不加字段（不给会话添噪音）。

**验证**：新增离线套件 `tools/verify-shots-recycle.mjs`（22 项：目录缺失/恰好超额边界/超期独立于数量/
重名不覆盖/非 PNG 不误伤/真 apply() 接线断言+真实时钟收敛）；README 门禁表与套件数同步（13⇒14 套）；
`npm test` 全绿。

## 0.15.0 — 2026-09-14 · 独立页 /bl/view 同样按"画面在更新"点灯

**问题**（与 v0.13.0 同源，用户 2026-09-14）：独立网页观察窗的内联脚本里，
`frame` 事件一到就 `dot.classList.add("on")`，`offline` 才灭；`es.onerror` 是空函数，
而且它的 2.5s 轮询里写着 `dot.classList.toggle("on", !!st.alive)` —— **宿主说浏览器活着就点绿**，
断流后不但不灭，还会被轮询重新点亮。独立页常被丢到副屏/全屏盯，所以"看着在播、其实早停了"更误导。

**改法**：把内嵌面板（v0.13.0）那套口径搬过去 —— `lastFrameAt` 记最后帧时刻，
`render()` 是唯一决定灯色的地方：绿=3.5s 内有帧、黄=在跑但断流（顶部写「正在重连」/「还没收到画面」，
画面上盖「⏸ 画面已停 · 最后更新于 X 秒前」）、灰=浏览器没跑；轮询只更新 `st` 再调 `render()`。
独立页没有"收起成一条"的形态，所以没有暂停态（暂停只在内嵌面板里）。

**验证**：`verify-panel-health` 30 ⇒ **34 项**（新增 4 条针对 host `VIEW_PAGE` 的接线断言：
记 lastFrameAt、有 render() 单点决定、轮询不再按 `!!st.alive` 点绿、有 stale 角标与状态文字位）；
`npm test` 语法门禁 8/8 + 套件 13/13 通过。**host 半要重启应用**才看到新页面。

## 0.14.0 — 2026-09-14 · 收起成一条 = 暂停画面流（展开立即恢复）

**问题**（用户 2026-09-14 提出的省资源项）：`setMin()` 只切了 CSS 类（`.bl-min`）与位置，
**画面流照旧在跑** —— 收起成一条把手后仍在拉 JPEG 帧、仍在设 `img.src`（解码也照做）。
只有整块面板被 ✕ 收走（`hidePanel`）才会关流。

**核实**：宿主那条 `GET /bl/stream?fps&q` 的注释写着"**无查看者时零开销**"（index.js:30），
即**断开订阅就真的停掉截图**（不是只省传输）—— 所以这条改在 client 侧就能吃到全部收益，
不需要动 host、不需要重启应用。

**改法**：新增 `applyStreamState()`：`want = S.open && !S.min`，要就拉流、不要就 `closeStream()`；
`setMin()` 与 `showPanel()` 都走它（showPanel 原来是无条件 `openStream()`）。
再加一个 `paused` 事实进健康判定：`computeHealth({paused})` ⇒ 收起态返回 `paused`（**灰实心灯**，
文案「已暂停（收起中）」），**不会**误报成黄灯"正在重连" —— 暂停是我们主动做的，不是断流。
收起态 `updateMinTitle()` 照旧显示当前页面名（`/bl/state` 轮询 2.5s 继续跑，只是不拉画面）。

**实测**（真浏览器）：收起 ⇒ 灯 `bl-dot paused`、文案「已暂停（收起中）」、隔 4.5 秒取帧字节
**一字未变**（画面真的停）；展开 ⇒ 灯回 `bl-dot on`、帧字节变化（立即恢复）；
收起态标题仍跟着页面走。测试：`verify-panel-health` 21 ⇒ **30 项**（新增暂停四态 + 六条接线断言：
`applyStreamState` 存在、setMin/showPanel 都过它、showPanel 不再无条件拉流、
CSS 有 `.bl-dot.paused`、healthState 传 paused）。
