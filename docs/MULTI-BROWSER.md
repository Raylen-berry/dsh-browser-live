...
5. 需要真点击/打字时再打开「允许操作」。
0.6 （v0.8.2）**改过扩展代码之后，必须在扩展页点一次「重新加载」** —— 未打包扩展不会自己跟着磁盘上的文件变，
   否则你以为装上了新版，跑的其实还是旧版（0.8.1 的"先导航再附加"就属于"不重载不生效"）。
0.7 （v0.8.3）**当前页未授权、目标站已授权时，改为「新开一个标签页」**（v0.8.1 是就地导航，会把你正在看的页面换掉）。
   新标签页落到已授权 origin 之后才 attach；你原来的标签页不动。
0.8 （v0.8.3）**Chrome 起不来的常见原因是安全软件拦了它的 GPU 进程沙箱**（本机实测：火绒 HipsDaemon；
   Edge 因微软签名被放行）。插件会**自动补试 `--in-process-gpu`**（渲染器沙箱仍保留）并记住成功组合；
   也可把 `chromePath` 指向 Edge。注：v0.8.2 曾把这件事错误归因于"DSH 以管理员运行"，已在 v0.8.3 更正。
