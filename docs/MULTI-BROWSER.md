...
0.7 （v0.8.2）**DSH 以管理员身份运行时 Chrome 起不来**：Chrome 无法在提权父进程下初始化沙箱，于是静默退出
   （加 `--no-sandbox` 才起，但默认不关沙箱 —— 拿它当 agent 浏览器本来就是安全折衷）；Edge 不受影响。
   插件的应对是**自动降级**到下一个候选浏览器（`chromePath` → 环境变量 → Chrome → Edge → Brave），
   并在观察窗写明「谁起不来、改用谁了」。
